...
@app.command()
def render(graph_path: Path, output_path: Path, method: RenderMethod = RenderMethod.SRIP2, dpi: int = 100, normalize_height: bool = False) -> None:
    graph = ab.load.file(graph_path)
    _render(graph, output_path, normalize_height, method, dpi)


def _render(graph: ab.Graph, output_path: Path, normalize_height: bool, method: RenderMethod = RenderMethod.SRIP2, dpi: int = 100) -> None:
    ...
    if method == RenderMethod.SRIP2:
        ...
        width = 10 / len(roots)
        images = [logical_render(graph, root, outer_width=width, dpi=dpi) for root in roots]
        image = images[0]
        if len(images) > 1:
            image = _concat_images(images)
        image.save(output_path)
        ...
        width = 10 / len(roots)
        images = [visualize_treemap_inmem(graph, root, width=width, dpi=dpi) for root in roots]
        image = images[0]
        if len(images) > 1:
            image = _concat_images(images)
        image.save(output_path)
