...
def scaling_study(
    ...
    results_path: Annotated[
        Path, typer.Argument(help="Path where the results JSON should be stored")
    ],
    ...
    for i in tqdm(range(execute_num_times)):
        for file in glob(requests_graphs_glob):
            ...
            start = time()
            render(graph, path, normalize_height=True)
    ...
    render(case_graph, case_path, normalize_height=True)
