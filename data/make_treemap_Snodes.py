...
colors = {"Attack": "#e6194B",
    "Support": "#3cb44b"}

def get_treemap_rects(tree, x, y, width, height, horizontal, depth=0) -> list:
    parts = len(tree["children"])
    if parts == 0:
        return [{"x": x, "y": y, "width": width, "height": height, "depth": depth, "label": tree["label"]}] if "label" in tree else []
    if "label" in tree:
        ...
        res = [{"x": x, "y": y, "width": own_width, "height": own_height, "depth": depth, "label": tree["label"]}]
        if horizontal:
            y += own_height
        else:
            x += own_width
...
def _get_children(graph: ab.Graph, node: ab.AtomNode):
    res = [child for child in graph.incoming_nodes(node) if isinstance(child, ab.SchemeNode)]
...
def build_tree(graph: ab.Graph, root: ab.AtomNode):
    if not isinstance(root, ab.SchemeNode):
        return {"children": [build_tree(graph, child) for child in _get_children(graph, root)]}
    if len(graph.incoming_nodes(root)) == 0:
        return {"id": root.id, "label": root.label, "children": []}
    return {"id": root.id, "label": root.label, "children": [build_tree(graph, child) for child in _get_children(graph, root)]}
...
def draw_treemap(rects, height, width, savepath):
    ...
    for rect in rects:
        if 'label' not in rect and rect["label"] != None:
            continue
        ax.add_patch(patches.Rectangle((rect["x"], rect["y"]), rect["width"], rect["height"], fill=True, facecolor=colors.get(rect["label"], "white"), linewidth=1, edgecolor="black"))
...
def standard_resize(file: str):
    img = Image.open(file)
    img = img.crop((img.width/2-177, img.height/2-182, img.width/2+192, img.height/2+177)).resize((256, 256))
    img.save(file)